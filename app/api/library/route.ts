...
export async function GET(request: Request) {
  ...
  if (hasPostgres()) {
    ...
    const documents = await sql`
      ...
      WHERE canonical_title ILIKE ${pattern}
        OR COALESCE(citation, '') ILIKE ${pattern}
        OR document_type ILIKE ${pattern}
      ORDER BY canonical_title ASC
      LIMIT 50
    `;
    return Response.json({ documents, query, persistence: "postgres" });
  }
